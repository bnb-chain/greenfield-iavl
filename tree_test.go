...
func TestVersionedRandomTreeSmallKeys(t *testing.T) {
	...
	singleVersionTree.SaveVersion(true)
...
func TestVersionedTree(t *testing.T) {
	...
	hash1, v, err := tree.SaveVersion(true)
	require.NoError(err)
	require.False(tree.IsEmpty())
	require.EqualValues(1, v)
	...
	hash2, v2, err := tree.SaveVersion(true)
	require.NoError(err)
	...
	hash3, v3, _ := tree.SaveVersion(true)
	...
	hash4, _, _ := tree.SaveVersion(true)
...
func TestVersionedTreeHash(t *testing.T) {
	...
	hash1, _, err := tree.SaveVersion(true)
	require.NoError(err)
...
func Benchmark_GetWithIndex(b *testing.B) {
	...
	b.Run("regular", func(sub *testing.B) {
		...
		_, latestVersion, err := t.SaveVersion(true)
		require.NoError(b, err)
